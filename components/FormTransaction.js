...
export default function FormTransaction({ onTambah }) {
  const [form, setForm] = useState({
    ...
    tipe: "pengeluaran",
    kategori: "makanan",
    // Inisialisasi state form tanpa nilai tanggal default untuk menghindari mismatch SSR/CSR
    tanggal: "", // tanggal diisi di useEffect agar hanya di client
  });

  // Set tanggal default hanya di client setelah komponen mount
  // Ini mencegah mismatch antara server dan client
  useEffect(() => {
    setForm((prev) => ({
      ...prev,
  ...
  function handleSubmit(e) {
    ...
    onTambah({
      ...
      // id transaksi dibuat di sini (client only) untuk menghindari mismatch SSR/CSR
      id: Date.now(),
    });
    // Reset form, tanggal diisi ulang di client agar konsisten
    setForm((prev) => ({
      keterangan: "",
      nominal: "",
      tipe: "pengeluaran",
